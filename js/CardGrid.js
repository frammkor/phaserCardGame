...
export default class CardGrid extends CardBase {
  constructor(data) {
    // type will let us change the propurse of the card
    let { value, type } = data;
    super(data);
    this.textValue = new Phaser.GameObjects.BitmapText(this.scene, 0, -100, 'pressstart', value);
    // add is a method in a PhaserComponent
  ...
  // set highlith as a starting point for cards interactions
  set highlighted(highlith) {
    if (highlith) {
      let color = 0xcccc88;
